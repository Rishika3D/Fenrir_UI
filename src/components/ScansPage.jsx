  ...
  return (
    <div className="flex items-center flex-1 last:flex-none">
      ...
      {index < stepDefs.length - 1 && (
        <div className={`h-0.5 w-12 sm:w-16 md:w-24 lg:w-32 mx-2 md:mx-3 mb-5 shrink-0 transition-all ${lineCls}`} />
      )}
    </div>
  )
}
...
/* ── Component ──────────────────────────────────────────────── */
const ScansPage = ({ navigate, darkMode = true, toggleDarkMode, showToast }) => {
  ...
  return (
    <div className={`h-screen flex overflow-hidden ${bg}`}>
      <Sidebar activePage="scans" navigate={navigate} darkMode={darkMode} mobileOpen={mobileOpen} setMobileOpen={setMobileOpen} />

      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">

        {/* ── Page header ── */}
        <div className={`flex flex-col sm:flex-row sm:items-center justify-between px-4 lg:px-6 py-3 sm:py-4 border-b ${border} shrink-0 gap-3 sm:gap-0`}>
          <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto overflow-hidden">
            <button
              className="lg:hidden p-1 -ml-1 text-gray-500 hover:text-gray-700 cursor-pointer shrink-0"
            ...
            <h1 className="text-lg font-bold shrink-0">Scan</h1>
            <div className={`flex items-center gap-1.5 text-sm overflow-x-auto no-scrollbar whitespace-nowrap ${subText}`}>
              <div className="shrink-0"><HomeIcon /></div>
              <div className="shrink-0"><ChevronRight /></div>
              <span className="shrink-0">Private Assets</span>
              <div className="shrink-0"><ChevronRight /></div>
              <span
                className="text-teal-500 font-medium hover:underline cursor-pointer shrink-0"
                onClick={() => navigate('dashboard')}>
                Web App Servers
              </span>
              <div className="shrink-0"><ChevronRight /></div>
              <span className={`shrink-0 ${subText}`}>google.com</span>
            </div>
          </div>
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            {toggleDarkMode && (
              <button onClick={toggleDarkMode}
                className={`hidden sm:flex w-8 h-8 rounded-lg items-center justify-center transition-colors cursor-pointer ${darkMode ? 'bg-white/5 text-gray-400 hover:bg-white/10 hover:text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}>
                {darkMode ? <SunIcon /> : <MoonIcon />}
              </button>
            )}
            <button
              onClick={() => showToast('Report exported successfully!')}
              className={`px-3 py-1.5 sm:px-4 sm:py-2 text-sm font-medium rounded-lg border transition-colors cursor-pointer ${darkMode ? 'border-[#2a2a2a] text-gray-300 hover:bg-white/5' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}>
              Export
            </button>
            <button
              onClick={() => showToast('Scan stopped', 'error')}
              className="px-3 py-1.5 sm:px-4 sm:py-2 text-sm font-medium rounded-lg border border-red-400 text-red-400 hover:bg-red-500/10 transition-colors cursor-pointer">
              Stop
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-auto">
          ...
          {/* ── Scan progress card ── */}
          <div className={`mx-4 lg:mx-6 mt-4 lg:mt-5 rounded-xl border ${cardBg}`}>
            {/* Top row: progress circle + steps */}
            <div className={`flex flex-col sm:flex-row items-center gap-6 px-4 py-6 lg:px-6 lg:py-6 border-b ${border}`}>
              ...
              {/* Vertical divider */}
              <div className={`hidden sm:block w-px self-stretch mx-2 ${darkMode ? 'bg-[#2a2a2a]' : 'bg-gray-200'}`} />

              {/* Step indicators */}
              <div className="w-full sm:w-auto overflow-x-auto no-scrollbar pb-2 sm:pb-0">
                <div className="flex items-center justify-start sm:justify-center min-w-max px-2 sm:px-0 flex-1">
            ...
            {/* Bottom row: metadata */}
            <div className="flex items-center justify-start overflow-x-auto no-scrollbar py-4 px-2 lg:px-0">
              {metaItems.map((m) => (
                <div key={m.label} className="flex-none sm:flex-1 px-4 lg:px-6 whitespace-nowrap border-r last:border-0 border-transparent sm:border-transparent">
          ...
          {/* ── Console + Finding log ── */}
          <div className="mx-4 lg:mx-6 mt-4 mb-4 flex flex-col lg:flex-row gap-4" style={{ minHeight: 480 }}>

            {/* Live scan console */}
            <div className={`flex-1 flex flex-col rounded-xl border overflow-hidden ${cardBg} min-h-[380px] lg:min-h-0`}>
              {/* Console title bar */}
              ...
              <div className={`flex items-center gap-0 border-b ${border} shrink-0`}>
                {[
                  ...
                  <button
                    ...
                    className={`px-4 py-2 text-xs font-medium border-b-2 transition-colors cursor-pointer ${activeTab === tab.id
            ...
            {/* Finding log */}
            <div className={`w-full lg:w-80 flex flex-col rounded-xl border overflow-hidden ${cardBg} min-h-[300px] lg:min-h-0`}>
        ...
        {/* ── Footer stats bar ── */}
        <div className={`flex items-center gap-0 border-t ${border} px-4 lg:px-6 py-3 text-xs shrink-0 overflow-x-auto no-scrollbar ${darkMode ? 'bg-[#141414]' : 'bg-white'}`}>
          {footerStats.map((s, i) => (
            <div key={i} className={`flex items-center justify-center whitespace-nowrap gap-1.5 pr-4 mr-4 ${i < footerStats.length - 1 ? `border-r ${border}` : ''}`}>
